...
float distance_final, distance_initial;
int target_angle;
int target_velocity_right, target_velocity_left;

void forward(int distance, int value)
{
  command = move_forward;
  target_angle = angle;
  distance_initial = distanceL;
  distance_final = distanceL + distance;
  target_velocity_left = value;
  target_velocity_right = value;

  analogWrite(PIN_TTL_LEFT_0, target_velocity_left);
  analogWrite(PIN_TTL_LEFT_1, 0);
  analogWrite(PIN_TTL_RIGHT_0, target_velocity_right);
  analogWrite(PIN_TTL_RIGHT_1, 0);
...
ISR(TIMER4_COMPA_vect)
{
  if (command == move_forward)
  {
    ...
    */
    if(((angle <= target_angle + 2) && (angle >= target_angle - 2)))
    {
      Serial.println("faster");
      target_velocity_left += 1;
      target_velocity_right += 1;
      analogWrite(PIN_TTL_LEFT_0, target_velocity_left);
      analogWrite(PIN_TTL_LEFT_1, 0);
      analogWrite(PIN_TTL_RIGHT_0, target_velocity_right);
      analogWrite(PIN_TTL_RIGHT_1, 0);
    ...
    else if(angle > target_angle + 2)
    {
      Serial.println("slow left");
      target_velocity_left -= 3;
      //target_velocity_right -= 1;
      analogWrite(PIN_TTL_LEFT_0, target_velocity_left);
      analogWrite(PIN_TTL_LEFT_1, 0);
      analogWrite(PIN_TTL_RIGHT_0, target_velocity_right);
